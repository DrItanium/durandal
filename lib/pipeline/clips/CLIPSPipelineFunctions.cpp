   ...
   if(passesLength > 0) {
      ...
      for(int i = GetDOBegin(passesDO); i <= passesEnd; ++i) {
         if(GetMFType(passesMultifield, i) == SYMBOL) {
            ...
            std::string str(tmpName);
            const char* c = str.c_str();
            IndirectPassHeader* header = indirectRegistry.getIndirectPassHeader(c);
            if(header) { //It is an indirect header
               llvm::Pass* p = indirectRegistry.createPass(llvm::StringRef(c));
               if(!p) {
                  EnvPrintRouter(theEnv, werror, msg("The given pass "));
                  EnvPrintRouter(theEnv, werror, msg(c));
                  EnvPrintRouter(theEnv, werror, 
                        msg(" exists but for some reason wasn't created\n"));
                  return;
               } else {
                  //A bit of a cheat but it will work because all CLIPSTemplatePasses
                  //are also of type CLIPSPass
                  pipeline::clips::CLIPSModulePass* cp = 
                     static_cast<pipeline::clips::CLIPSModulePass*>(p);
                  cp->setEnvironment(theEnv);
                  PM.add(p);
               }
            } else { //it isn't an indirect header (native)
               const llvm::PassInfo* pass = registry.getPassInfo(llvm::StringRef(c));
               if(pass) {
