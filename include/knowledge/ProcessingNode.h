...
void* ProcessingNode<T,Pass>::getInstanceName(void* env, T* inst, Pass& p) {
	return EnvAddSymbol(env, EnvGetInstanceName(env, dispatch(env, inst, p)));
}

template<typename T, typename Pass>
void* getInstanceName(void* theEnv, T* instance, Pass& pass) {
	return ProcessingNode<T,Pass>::getInstanceName(theEnv, instance, pass);
}

template<typename T, typename Pass>
void setParent(void* theEnv, T* inst, Pass& pass) {
	ProcessingNode<T,Pass>::setParent(theEnv, inst, pass);
}

template<typename T, typename Pass>
void ProcessingNode<T,Pass>::setParent(void* theEnv, T* target, Pass& pass) {
	DATA_OBJECT wrapper;
...
template<typename T, typename Pass>
void ProcessingNode<T,Pass>::populateInstance(void* env, T* instance, Pass& pass) { }

template<typename T, typename Pass>
void ProcessingNode<T,Pass>::buildInstance(llvm::raw_string_ostream& str, void* theEnv, T* data, Pass& pass) { }
...
struct ProcessingNode<llvm::BasicBlock, Pass> {
...
template<>
struct ProcessingNode<llvm::BasicBlock,llvm::BasicBlockPass> {
...
template<typename Pass>
struct ProcessingNode<llvm::Argument, Pass> {
...
template<typename Pass>
struct ProcessingNode<llvm::Module, Pass> {
	...
	static void populateInstance(void* theEnv, llvm::Module* data, Pass& pass) { 

	}
};

template<typename Pass>
struct ProcessingNode<llvm::Function, Pass> {
	static void buildInstance(llvm::raw_string_ostream& str, void* theEnv, llvm::Function* data, Pass& pass) {
		ProcessingNode<Pass, llvm::GlobalObject>::buildInstance(str, theEnv, (llvm::GlobalObject*)data, pass);
	...
	static void populateInstance(void* theEnv, llvm::Function* data, Pass& pass) {
		ProcessingNode<Pass, llvm::GlobalObject>::populateInstance(theEnv, (llvm::GlobalObject*)data, pass);
	}
};
