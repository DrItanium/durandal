...
#define DefineCustomDispatch(type, envName, instName) \
	template<typename P> \
	void* dispatch(void* envName, type* instName) 
#define CondDispatch(type, env, val) \
	if (type* v = llvm::dyn_cast<type>(val)) return dispatch<P>(env, v)

DefineCustomDispatch(llvm::TerminatorInst, env, inst) {
...
DefineCustomDispatch(llvm::UnaryInstruction, env, inst) {
	WhenInstanceDoesNotExist(env, inst) {
...
DefineCustomDispatch(llvm::CmpInst, env, inst) {
	WhenInstanceDoesNotExist(env, inst) {
...
DefineCustomDispatch(llvm::Instruction, env, inst) {
	WhenInstanceDoesNotExist(env, inst) {
...
DefineCustomDispatch(llvm::User, env, inst) {
	WhenInstanceDoesNotExist(env, inst) {
...
DefineCustomDispatch(llvm::Value, env, inst) {
	WhenInstanceDoesNotExist(env, inst) {
...
DefineCustomDispatch(llvm::CastInst, env, inst) {
	WhenInstanceDoesNotExist(env, inst) {
...
//DefineCustomDispatch(llvm::IntrinsicInst, env, inst) {
//	WhenInstanceDoesNotExist(env, inst) {
//		CondDispatch(llvm::DbgInfoIntrinsic, env, inst);
//		Otherwise(llvm::IntrinsicInst, env, inst);
//	}
//}
DefineCustomDispatch(llvm::CallInst, env, inst) {
	WhenInstanceDoesNotExist(env, inst) {
